    ...
    if (attr(formula, "splines")$noncentered) {
        ...
        for (i in seq_along(formula)) {
            ...
            if (is_categorical(formula[[i]]$family)) {
                ...
                tau_term <- c(idt(1), "vector<lower=0>[K_", i, "] tau_", i, ";")
            } else {
        ...
        for (i in seq_along(formula)) {
            ...
            if (is_categorical(formula[[i]]$family)) {
                ...
                tau_term <- c(idt(1), "vector<lower=0>[K_", i, "] tau_", i, ";")
                beta_term <- c(idt(1), "matrix[K_", i, ", S_", i, " - 1] beta_mean_", i, ";")
            } else {
                a_term <- c(idt(1), "row_vector[D] a_", i, "[K_", i, "];")
                tau_term <- c(idt(1), "vector<lower=0>[K_", i, "] tau_", i, ";")
                beta_term <- c(idt(1), "vector[K_", i, "] beta_mean_", i, ";")
            }
            mtext <- paste_rows(mtext, a_term, tau_term, beta_term)
            if (is_gaussian(formula[[i]]$family)) {
                sigma_term <- c(idt(1), "real<lower=0> sigma_", i, ";")
                mtext <- paste_rows(mtext, sigma_term)
            }
        ...
        if (attr(formula, "splines")$noncentered) {
            ...
            if (is_categorical(formula[[i]]$family)) {
                spline_term <- paste_rows(
                    ...
                    c(idt(2), "for (k in 1:K_", i, ") {"),
                    c(idt(3), "for (t in 1:T) {"),
                    c(idt(4), "beta_", i, "[t, k, s] = beta_mean_", i, "[k, s] + a_", i, "[s, k] * Bs[, t];"),
                    c(idt(3), "}"),
                    c(idt(2), "}"),
                    c(idt(1), "}"),
                    c(idt(1), "for (t in 1:T) {"),
                ...
                spline_term <- paste_rows(
                    c(idt(1), "for (k in 1:K_", i, ") {"),
                    c(idt(2), "for (t in 1:T) {"),
                    c(idt(3), "beta_", i, "[t, k] = beta_mean_", i, "[k] + a_", i, "[k] * Bs[, t];"),
                    c(idt(2), "}"),
                    c(idt(1), "}")
                )
            }
...
create_model <- function(formula, idt, ...) {
    ...
    for (i in seq_along(formula)) {
        if (is_categorical(formula[[i]]$family)) {
            ...
            c(priors) <- paste0(idt(1), "to_vector(beta_mean_", i, ") ~ normal(0, 2);")
        } else {
            if (attr(formula, "splines")$noncentered) {
                a_term <- paste_rows(
                    c(idt(1), "for (k in 1:K_", i, ") {"),
