...
* Added a new argument `include_varying` for `as.data.frame()`, `as.data.table()`, and `as_draws()` methods for `dynamitefit` objects. If `FALSE`, all time-varying parameters are omitted from the output.
