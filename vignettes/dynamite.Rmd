...
The output from `predict` is a `list` with two components, simulate: the first contains
the predicted values in a data frame with the columns `g_new`, `p_new`, and 
`b_new` where the NA values of `newdata` are 
...
`logp` and index variable `.draw`), the second components contains 
the original predictors of `newdata` :

```{r}
head(pred0$simulated)
```
