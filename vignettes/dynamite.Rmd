...
methods. The main features distinguishing the package and the underlying 
methodology from many other approaches are:

* Support for effects which vary smoothly over time. This allows modelling for 
  example effects of interventions which increase or decrease over time.
* Support for non-Gaussian observations. `dynamite` supports not only Gaussian 
  response variables, but also many other common types such as categorical 
  variables.
...
* Transparent quantification of parameter and predictive uncertainty due to the 
  use of Bayesian estimation methods. 

As we are modelling all endogenous variables jointly, it is also possible to 
consider long-term effects of interventions which take into account the 
interdependency of several variables of the model, and allows simulation of 
