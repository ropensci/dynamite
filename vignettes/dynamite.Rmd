...
Let's first fit the basic model without time-invariant effects of value and capital on the investment, but with a firm-specific random intercept (using the `random_intercept = TRUE` inside of `obs`:
```{r, eval = FALSE}
fit_basic <- dynamite(obs(invest ~ value + capital,
                          family = gaussian(), random_intercept = TRUE),
                d, "firm", "year",
                chains = 2, cores = 2, iter = 2000)
fit_basic
```
The results are close to the ones obtained with the `plm` package:
