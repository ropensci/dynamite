...
`id` (50 unique firms), the `time` (20 time points), a continuous variable `g`, a variable with non-negative integer values `p`, and a binary variable `b`. We define a following model (which actually matches the data generating process used in simulating the data):
