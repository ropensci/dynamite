...
functions {
  ...
  */
  vector forward_algorithm(int[,] y, int N, int K, int T, matrix logA, matrix logB) {

    matrix[K, T] log_alpha;
    log_alpha[1, 1] = sum(logB[1, y[1, ]]);
    // for(i in 1:N) {
    //   log_alpha[1, 1] += logB[1, y[1, i]];
    // }
    log_alpha[2:K, 1] = rep_vector(negative_infinity(), K - 1);

    for (t in 2:T) {
      // stay at the starting state 1
      log_alpha[1, t] = log_alpha[1, t - 1] + logA[1, 1] + sum(logB[1, y[t, ]]);
     // for(i in 1:N) log_alpha[1, t] += logB[1, y[t, i]];
      // move (or stay) on state 2 to K
      for (k in 2:K) {
        real log_py = sum(logB[k, y[t, ]]);
        real move = log_alpha[k - 1, t - 1] + logA[k - 1, 2] + log_py;
        real stay = log_alpha[k, t - 1]   + logA[k, 1] + log_py;
...
transformed parameters {
  ...
  // for(n in 1:N) {
  //   loglik[n] = log_sum_exp(forward_algorithm(y[, n], N, K, T, logA, logB));
  // }
}
model {
